...
class _PrayerAdhanSettingsScreenState extends State<PrayerAdhanSettingsScreen> {
  ...
  String get voice => widget.prefs.getString('adhanVoice') ?? 'الأذان المكي';
  String get method => widget.prefs.getString('prayerMethod') ?? 'الهيئة المصرية العامة للمساحة';

  final voices = const ['الأذان المكي', 'أذان المدينة المنورة', 'أذان هادئ', 'صوت افتراضي'];
  final methods = const ['الهيئة المصرية العامة للمساحة', 'رابطة العالم الإسلامي', 'أم القرى'];

  Future<void> setBool(String key, bool value) async { await widget.prefs.setBool(key, value); if (mounted) setState(() {}); }
  Future<void> setString(String key, String value) async { await widget.prefs.setString(key, value); if (mounted) setState(() {}); }
  Future<void> setInt(String key, int value) async { await widget.prefs.setInt(key, value); if (mounted) setState(() {}); }

  @override
  Widget build(BuildContext context) => Scaffold(
    body: Column(children: [
      IslamicHeader(title: 'مواقيت الصلاة والأذان', subtitle: 'خصص تنبيهات الصلاة وصوت الأذان'),
      Expanded(child: ListView(padding: const EdgeInsets.fromLTRB(16, 16, 16, 40), children: [
        _SettingTile(icon: Icons.notifications_active_rounded, title: 'تفعيل تنبيهات الصلاة', subtitle: 'إظهار تنبيه عند دخول وقت الصلاة', value: prayerNotifications, onChanged: (v) => setBool('prayerNotifications', v)),
        _SettingTile(icon: Icons.volume_up_rounded, title: 'تشغيل الأذان', subtitle: 'تشغيل صوت الأذان عند التنبيه', value: adhanEnabled, onChanged: (v) => setBool('adhanEnabled', v)),
        _SettingTile(icon: Icons.alarm_rounded, title: 'تنبيه قبل الصلاة', subtitle: 'تذكير قبل دخول وقت الصلاة', value: preReminder, onChanged: (v) => setBool('prePrayerReminder', v)),
        if (preReminder) _ChoiceCard<int>(icon: Icons.schedule_rounded, title: 'موعد التذكير', value: reminderMinutes, values: const [5, 10, 15, 20], label: (v) => 'قبل الصلاة بـ $v دقائق', onChanged: (v) => setInt('prePrayerMinutes', v)),
        _ChoiceCard<String>(icon: Icons.record_voice_over_rounded, title: 'صوت الأذان', value: voice, values: voices, label: (v) => v, onChanged: (v) => setString('adhanVoice', v)),
        _SettingsButton(icon: Icons.play_circle_outline_rounded, title: 'تجربة صوت الأذان', subtitle: 'الصوت المختار: $voice', onTap: () => ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('تم اختيار $voice. أضف ملفاته الصوتية إلى assets لتشغيل المعاينة والأذان فعلياً.', style: GoogleFonts.cairo())))),
        _ChoiceCard<String>(icon: Icons.calculate_rounded, title: 'طريقة حساب المواقيت', value: method, values: methods, label: (v) => v, onChanged: (v) => setString('prayerMethod', v)),
      ])),
    ]),
  );
}
...
class AppLanguageScreen extends StatefulWidget { final SharedPreferences prefs; const AppLanguageScreen({super.key,required this.prefs}); @override State<AppLanguageScreen> createState()=>_AppLanguageScreenState(); }
class _AppLanguageScreenState extends State<AppLanguageScreen>{
  String get lang=>widget.prefs.getString('appLanguage')??'العربية';
  Future<void> choose(String v) async { await widget.prefs.setString('appLanguage',v); if(mounted)setState((){}); }
  @override Widget build(BuildContext context)=>Scaffold(body:Column(children:[IslamicHeader(title:'اللغة',subtitle:'اختر لغة واجهة التطبيق'),Expanded(child:ListView(padding:const EdgeInsets.all(16),children:['العربية','English'].map((v)=>RadioListTile<String>(value:v,groupValue:lang,onChanged:(x)=>choose(x!),title:Text(v,style:GoogleFonts.cairo(fontWeight:FontWeight.bold,color:_navy)),secondary:Icon(v=='العربية'?Icons.translate:Icons.language,color:_gold))).toList()))]));
}
